...
Dim fileList(1024) As String * 1024
    ...
    Else
        ...
        For loopN = 0 To finalCount Step 1
            ...
            For I = 0 To fileN
                ...
                End If
                If FRAMESKIP = -1 And I Mod 2 = 1 Then GoTo SkipDisplay
                If _FileExists(_Trim$(fileList(I))) Then
                    a = _LoadImage(_Trim$(fileList(I)))
                    widthImage = _Width(a)
                    ...
                    End If
                Else
                    Write_log "    File does not exist, skipping display"
                    GoTo SkipDisplay
                End If
                '------Trim---------------------------------------------------------
                If trimFile Then
                    Cls , rgb
                    _PutImage (OffsetX + ScaleFactor * trim(I).X, OffsetY + ScaleFactor * trim(I).Y)-(OffsetX + ScaleFactor * trim(I).X + ScaleFactor * trim(I).W, OffsetY + ScaleFactor * trim(I).Y + ScaleFactor * trim(I).H), a
                Else
                    Cls , rgb
                    _PutImage (OffsetX + ScaleFactor * (W - widthImage) / 2, OffsetY + ScaleFactor * (H - heightImage) / 2)-(OffsetX + ScaleFactor * (W + widthImage) / 2, OffsetY + ScaleFactor * (H + heightImage) / 2), a
                End If
                '-------------------------------------------------------------------
                ...
                If SAVE_MODE = 2 Then PNG "..\out\" + path$ + "\" + Left$(fileList(I), _InStrRev(fileList(I), ".")) + "png", a
                _Display
                _FreeImage a
                Do
                    _Limit 60
                Loop While _KeyDown(80) Or _KeyDown(112)
                K$ = InKey$
                If Len(K$) = 1 And UCase$(K$) <> "P" And count = 0 Then EndAnimation = -1: finalCount = loopN
                SkipDisplay:
